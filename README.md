# Desafio Técnico PRODAP — CRUD de Produtos
...
Isso já instala as dependências, aplica as migrations e sobe o servidor automaticamente.
Acesse **http://localhost:8000/** — você será redirecionado para `/produtos/`.
...
# 4. Aplique as migrations (cria o banco SQLite e a tabela "produtos")
python manage.py migrate

# 5. (Opcional) crie um usuário admin para acessar /admin/
...
## Decisões de projeto (observações)

- Sem Django REST Framework: optei por views nativas do Django retornando
  `JsonResponse` para os dois pontos que o desafio pede explicitamente em AJAX
  (listar e excluir). Isso evita uma dependência extra para um CRUD pequeno e
  deixa claro o que cada view está fazendo. Criação/edição usam formulários
...
- **Possíveis evoluções** (fora do escopo do desafio, mas que eu consideraria
  num projeto real): testes automatizados (`django.test`), paginação na listagem,
  e migrar a API para Django REST Framework se o projeto crescer.
